...
def base_label_process(root_path='data/JKJ1A-dataset/labels', metric='beauty', save=False):
    ...
    for path in sorted(_get_label_paths(root_path)):
        if 'csv' not in str(path): continue 
        print(path)
...
if __name__ == '__main__':
    import os 
    # print(os.getcwd())
    base_label_process(root_path='data/JKJ1A-dataset/labels', metric='beauty2', save=True)
    base_label_process(root_path='data/JKJ1A-dataset/labels', metric='gross', save=True)